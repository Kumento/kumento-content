...
/**
 * The kumento_events Custom Meta Box class.
 *
...
class Kumento_Events_Meta_Box {
	...
	private $fields = [
        ...
        'kumento_content_venue_address' => [
			'type' => 'text',
			'label' => 'Venue Address',
			'default' => '',
		],
        'kumento_content_venue_address_second' => [
			'type' => 'text',
			'label' => 'Venue Address 2',
			'default' => '',
		],
		'kumento_content_venue_zip' => [
			'type' => 'number',
			'label' => 'Venue Zipcode',
			'default' => '',
		],
        'kumento_content_venue_city' => [
			'type' => 'text',
			'label' => 'Venue City',
			'default' => '',
		],
        ...
        'kumento_content_address_second' => [
			'type' => 'text',
			'label' => 'Organizer Address 2',
			'default' => '',
		],
		'kumento_content_zip' => [
			'type' => 'number',
			'label' => 'Organizer Zipcode',
			'default' => '',
		],
        ...
        'kumento_content_date' => [
			'type' => 'date',
			'label' => 'Event Date',
			'default' => '',
		],
		'kumento_content_starttime' => [
			'type' => 'time',
			'label' => 'Time, Event Start',
			'default' => '',
		],
		'kumento_content_endtime' => [
			'type' => 'time',
        ...
        'kumento_content_pricing' => [
			'type' => 'textarea',
			'label' => 'Pricing',
			'default' => '',
		],
        'kumento_content_homepage_signup' => [
			'type' => 'url',
			'label' => 'Signup Link',
			'default' => '',
		],
	...
	public function __construct() {
		add_action( 'add_meta_boxes', [ $this, 'kumento_events_register_meta_boxes' ] );
		add_action( 'save_post', [ $this, 'kumento_events_save_meta_box_fields' ] );
		add_action('rest_api_init', [ $this, 'kumento_register_events_post_meta_fields' ] );
	}
	 ...
	 * @return void
	 */
	public function kumento_events_register_meta_boxes() : void {
		foreach ( $this->display_locations as $location ) {
			add_meta_box(
				'kumento-content-events-meta-box', /* The id of our meta box. */
				__( 'Fields', 'kumento_content' ), /* The title of our meta box. */
				[ $this, 'kumento_events_render_meta_box_fields' ], /* The callback function that renders the metabox. */
	 ...
	 * @return void
	 */
	public function kumento_events_render_meta_box_fields(WP_Post $post) : void {
		wp_nonce_field( 'kumento-content-events-meta-box_data', 'kumento-content-events-meta-box_nonce' );
		$html = '';
		foreach( $this->fields as $field_id => $field ){
			...
			$field_html = $this->kumento_events_render_input_field( $field_id, $field, $meta_value );
			$label = "<label for='$field_id'>{$field['label']}</label>";
			$html .= $this->kumento_events_format_field( $label, $field_html );
		}
	 ...
	 * @return string
	 */
	public function kumento_events_format_field( string $label, string $field ): string {
	 ...
	 */
	public function kumento_events_render_input_field( string $field_id, array $field, string $field_value): string {
		switch( $field['type'] ){
			case 'select': {
	 ...
	 */
	public function kumento_events_save_meta_box_fields( int $post_id ) {
		if ( ! isset( $_POST['kumento-content-events-meta-box_nonce'] ) ) return;
	
		$nonce = $_POST['kumento-content-events-meta-box_nonce'];
		if ( !wp_verify_nonce( $nonce, 'kumento-content-events-meta-box_data' ) ) return;
	...
	public function kumento_register_events_post_meta_fields() {

		foreach ( $this->display_locations as $location ) {
			register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_organizer', // Name of field
				array(
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_venue_address', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_venue_address_second', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_venue_zip', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_venue_city', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_date', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_starttime', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_endtime', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_pricing', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
            ...
            register_rest_field(
				$location, // Object post|term|comment etc.
				'kumento_content_homepage_signup', // Name of field
				array(
					'get_callback'    => [ $this, 'kumento_get_all_post_meta' ],
					'update_callback'    => [ $this, 'kumento_update_post_meta' ],
					'schema'          => null, // Schema for the field
...
if ( class_exists( 'Kumento_Events_Meta_Box' ) ) {
	new Kumento_Events_Meta_Box();
}